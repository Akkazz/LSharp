...
namespace Firestorm_AIO.Champions.Yasuo
{
    public class Yasuo : Bases.ChampionBase
    {
        ...
        private static Obj_AI_Base GetBestEscapeE()
        {
            return
                GameObjects.Enemy.Where(
                    m =>
                        !HasEBuff(m) &&
                        m.IsValidTarget(E.Range)).OrderBy(m => m.Distance(Game.CursorPos))
        ...
        {
            return
                (int) GameObjects.EnemyHeroes.Where(a => a.IsKnockedUp() && a.IsValidTarget(R.Range)).Select(a =>
                {
        ...
        public override void Menu()
        {
            ...
            ComboMenu.Add(new MenuSlider("rCount", "Only R if there are X knockedup", 2, 0, 5));
        ...
        {
            Target = Variables.TargetSelector.GetTarget(R.Range, DamageType.Physical);
                ...
                if (!Me.IsDashing())
                {
                    ...
                    var minionE =
                        GameObjects.EnemyMinions.OrderBy(m => m.Health)
                            .FirstOrDefault(
                                m =>
                                    m.IsValidTarget(E.Range) && !HasEBuff(m) &&
                                    Health.GetPrediction(m, 30 + Game.Ping) <= E.GetDamage(m));
